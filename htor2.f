...
* Open files.
      ...
      open(unit=2,file='Morb4.d',status='UNKNOWN')  !LC
      open(unit=3,file='M-tot.d',status='UNKNOWN')
      open(unit=4,file='Morb3.d',status='UNKNOWN')  !IC
...
* phi-mesh!
      ...
      write(6,*)'Read IC contribution from file!'
      rewind(4)
      read(4,*)
      do ip = 1, nphi
      read(4,'(3f10.4)') rtemp1,MIC(ip)
      write(6,'(3f10.4)') rtemp1,MIC(ip)
      enddo
...
* Term1:
      ...
      ctem1  = ctem1 + dconjg(ukxp(i1))*ukn(i1,ib)
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*ukyp(i1)*
     .                 cdexp( xi*dy*ty(i1))
      enddo

      csum11 = csum11 + ctem1*ctem2*ekn(ib) 
...
* Term2:
      ...
      do i1  = 1,kdim
      ctem1  = ctem1 + dconjg(ukxp(i1))*ukn(i1,ib)
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*ukym(i1)*
     .                 cdexp(-xi*dy*ty(i1))
      enddo

      csum12 = csum12 - ctem1*ctem2*ekn(ib)
...
* Term3:
      ...
      do i1  = 1,kdim
      ctem1  = ctem1 + dconjg(ukxm(i1))*ukn(i1,ib)
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*ukyp(i1)*
     .                 cdexp( xi*dy*ty(i1))
      enddo

      csum13 = csum13 - ctem1*ctem2*ekn(ib)

      enddo
* Term4:
      csum14 = dcmplx(0.d0,0.d0)
     ...
     .                 cdexp(-xi*dy*ty(i1))
      ...
      csum14 = csum14 + ctem1*ctem2*ekn(ib)

      enddo
* Term5:
      csum15 = dcmplx(0.d0,0.d0)
      ...
      do i1  = 1,kdim
      ctem1  = ctem1 + dconjg(ukyp(i1))*ukn(i1,ib)*
     .                 cdexp(-xi*dy*ty(i1))
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*ukxp(i1)
      enddo

      csum15 = csum15 - ctem1*ctem2*ekn(ib)

      enddo
* Term6:
      csum16 = dcmplx(0.d0,0.d0)
      ...
      ctem1  = ctem1 + dconjg(ukyp(i1))*ukn(i1,ib)*
     .                 cdexp(-xi*dy*ty(i1))
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*ukxm(i1)
      enddo

      csum16 = csum16 + ctem1*ctem2*ekn(ib)

      enddo
* Term7:
      csum17 = dcmplx(0.d0,0.d0)
      ...
      do i1  = 1,kdim
      ctem1  = ctem1 + dconjg(ukym(i1))*ukn(i1,ib)*
     .                 cdexp( xi*dy*ty(i1))
      ctem2  = ctem2 + dconjg(ukn(i1,ib))*ukxp(i1)
      enddo

      csum17 = csum17 + ctem1*ctem2*ekn(ib)

      enddo
* Term8:
      csum18 = dcmplx(0.d0,0.d0)
     ...
     .                 cdexp( xi*dy*ty(i1))
      ...
      csum18 = csum18 - ctem1*ctem2*ekn(ib)
     ...
     .                        csum18)/(4.d0*dx*dy)
      ...
      if(ip.eq.1) write(6,'(a30,3f10.2)')'LC contribution to M:'
      if(ip.eq.1) write(2,'(a30,3f10.2)')'LC contribution to M:'
      write(6,'(a30,3f10.4)')'M:',phi/pi,-dimag(csum1*volwgt*fact)*2.d0 !FA factor 2
      write(2,'(3f10.4)'),phi/pi,-dimag( csum1*volwgt*fact )*2.d0         !FA factor 2


      write(3,'(3f10.4)'),phi/pi,
